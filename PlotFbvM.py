...
def PlotFbvM(inputfile):
    ...
    plt.errorbar(M_gal, Fb_gal_min, yerr=np.array([[0],[.007]]), uplims=True, mfc='b', mec='b', ms=12, ecolor='b', label='')
    plt.errorbar(10**(12.2), Fb_gal_max, yerr=np.array([[0.007],[0]]), lolims=True, mfc='b', mec='b', ms=12, ecolor='b', label='')
...
def SetAxes(legend=False):
    ...
    plt.text(8e+12,F_wmap+0.008, r'f$_{b,WMAP}$', verticalalignment='bottom', size='x-large')
    ...
    plt.text(8e+12,F_planck-0.008, r'f$_{b,Planck}$', verticalalignment='top', size='x-large')
    ...
    plt.xscale('log')
    plt.xlim([1e+12,1.6e+15])
    plt.ylim(ymin=0)

    plt.text(4e+12, 0.02, 'Galaxies', size='large', verticalalignment='center')
    plt.plot(3e+12, 0.02, 'ko', ms=12, mfc='k', mec='k', marker=r'$\Leftarrow$')

    plt.text(1.7e+13, 0.02, 'Groups & Clusters', size='large', verticalalignment='center')
    plt.plot(2e+14, 0.02, 'ko', ms=12, mfc='k', mec='k', marker=r'$\Rightarrow$')
